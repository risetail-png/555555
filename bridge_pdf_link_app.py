...
def add_buttons_bottom(pdf, page_idx, btn_list, page_w, page_h,
    ...
    """青・緑ボタン用: ページ下端（BTN_Y1〜BTN_Y2）に1行固定で配置。"""
    _place_button_row(pdf, page_idx, btn_list, page_w,
                      BTN_Y1, BTN_Y2,
                      fill_color, outline_color, font_path,
                      f'/{xobj_prefix}{page_idx}r0')
        ...
        for pp in sorted(target_photo_pages):
            ...
            if page_nums:
                if is_multi_span and pp_span:
                    label = (f"{pp_span}-{min(page_nums)}〜{max(page_nums)}"
                             if len(page_nums) > 1 else f"{pp_span}-{page_nums[0]}")
                else:
                    label = (f"{min(page_nums)}〜{max(page_nums)}"
